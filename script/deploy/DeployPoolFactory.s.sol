...
contract DeployPoolFactory is Script {
    ...
    function setUp() public {
        if (address(config) == address(0)) {
            ...
            priceFeed = config.getActivePriceFeed();
            vrfWrapper = config.getActiveVrf();
        }
    }
    ...
    function run() public returns (PoolFactory) {
        ...
        PoolFactory poolFactory =
            new PoolFactory(activeConfig.routerAddress, activeConfig.linkAddress, activeConfig.chainSelector, priceFeed);
